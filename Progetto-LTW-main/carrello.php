...
<head>
    ...
    <link rel="stylesheet" media="screen and (min-width: 1231px)" href="css/PC_carrello.css">
    <link rel="stylesheet" media="screen and (min-width: 601px) and (max-width: 1230px)" href="css/Tablet_carrello.css">
    <link rel="stylesheet" media="screen and (max-width: 600px)" href="css/Smartphone_carrello.css">
    ...
    <title>Carrello - Bestway Streetwear</title>
</head>
<body>
    ...
    <div class="carrello">
        ...
        <?php
            ...
            if ($dbconn) {
                ...
                if($op == "remove") {
                    $prodotto_code_rm = $_POST["prodotto_code"];
                    if($prodotto_code_rm != "") {
                        $prodotto_colore_rm = $_POST["prodotto_colore"];
                        $prodotto_taglia_rm = $_POST["prodotto_taglia"];
                        $q1 = "delete from carrello where utente = $1 and prodotto_code = $2 and prodotto_colore = $3 and prodotto_taglia = $4";
                        $result_rm = pg_query_params($dbconn, $q1, array($utente, $prodotto_code_rm, $prodotto_colore_rm, $prodotto_taglia_rm));
                    }
                    else {
                        $q1 = "delete from carrello where utente = $1";
                        $result_rm = pg_query_params($dbconn, $q1, array($utente));
                    }
                }


                $q1 = "select * from carrello where utente = $1";
                ...
                if($array) {
                    ...
                    echo '
                        <div class="buttons">
                            ...
                            <input type="text" name="utente" value="' . $utente . '" style="display: none;">
                            <input type="text" name="op" value="remove" style="display: none;">
                    ...
                    echo '<div class="nop">
                            <img class="pcimg" src="/Immagini/carrello_vuoto2.jpg">
                            <img class="phnimg" src="/Immagini/carrello_vuoto3.jpg">
                        </div>'
                        ;
                }
            }
    ...
    <?php
        ...
        function carrello($prodotti) {
            
            $utente = $_SESSION["utente"];

            $s = '';
            foreach ($prodotti as $index => $var) {
                $code = $var["prodotto_code"];
                $colore = $var["prodotto_colore"];
                $p = trova_prodotto($code, $colore);
                $s = $s . '
                <div class="prodotto_carrello">
                    <div class="imgprod">
                        <img src="immagini/Prodotti/' . $p["immagine1"] . '"></img>
                    </div>
                    <div class="textprod">
                        '
                        . 'Nome: ' . $p["nome"] . 
                        ...
                        <form action="carrello.php" method="post">
                            <input type="text" name="utente" value="' . $utente . '" style="display: none;">
                            <input type="text" name="op" value="remove" style="display: none;">
                            <input type="text" name="prodotto_code" value="'. $var["prodotto_code"] .'" style="display: none;">
                            <input type="text" name="prodotto_colore" value="'. $var["prodotto_colore"] .'" style="display: none;">
                            <input type="text" name="prodotto_taglia" value="'. $var["prodotto_taglia"] . '" style="display: none;">
                            <button class="btn btn-dark btn-rounded">
                                <input type="submit" style="display:none;">
                                Rimuovi
                            </button>
        ...
        function trova_prodotto($code, $colore) {
            if ($_SERVER["REQUEST_METHOD"] != "POST") {
                header("Location: /");
            }
            else {
        ...
        function totale($prodotti) {
            ...
            foreach ($prodotti as $index => $var) {
                ...
                $totale = $totale + $p["prezzo"] * $var["quantità"];
            ...
            if ($dbconn) {
                $utente = $_SESSION["utente"];
                $q1 = "select sconto from sconto where utente = $1";
                $result = pg_query_params($dbconn, $q1, array($utente));
                $sconto = pg_fetch_array($result, null, PGSQL_ASSOC);
            }

            if($sconto) {
                $totale_scontato = $totale - (($totale * 20) / 100);
                return '
                <div class="totale">
                    Totale: ' . $totale_scontato . ' $ (-20%)
                </div>';
            }
