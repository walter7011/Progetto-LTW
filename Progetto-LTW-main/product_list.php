...
<head>
    ...
    <link rel="stylesheet" media="screen and (min-width: 1231px)" href="css/PC_product_list.css">
    <link rel="stylesheet" media="screen and (min-width: 601px) and (max-width: 1230px)" href="css/Tablet_product_list.css">
    <link rel="stylesheet" media="screen and (max-width: 600px)" href="css/Smartphone_product_list.css">
    
    <link rel="icon" type="image/png" href="immagini/bws_logo.png">
	<link rel="shortcut icon" type="image/png" href="immagini/bws_logo.png">
    <title>Catalogo - Bestway Streetwear</title>
</head>
...
-->
    ...
    <div class="sconto">
        ...
        <?php
            ...
            if ($dbconn) {
                ...
                // cerca
                if($search != "") {
                    $q1 = "select * from prodotto where nome like '%" . $search . "%'";
                    $result = pg_query($dbconn, $q1);
                    $prodotti = pg_fetch_all($result, PGSQL_ASSOC);
                }
                    ...
                    // novità
                    if($novità) {
                        $q1 = "select * from prodotto where novità = $1";
                        $result = pg_query_params($dbconn, $q1, array($novità));
                        $prodotti = pg_fetch_all($result, PGSQL_ASSOC);
                    }
                        ...
                        // uomo/donna
                        if($categoria) {
                            $q1 = "select * from prodotto where categoria = $1 and sesso = $2";
                            $result = pg_query_params($dbconn, $q1, array($categoria, $sesso));
                            $prodotti = pg_fetch_all($result, PGSQL_ASSOC);
                        }

                        // categoria
                        else {
                            $q1 = "select * from prodotto where sesso = $1";
                            $result = pg_query_params($dbconn, $q1, array($sesso));
                            $prodotti = pg_fetch_all($result, PGSQL_ASSOC);
                        }
                    ...
                    // scarpe : categ. = AllScarpe
                    if($categoria == "AllScarpe") {
                        $q1 = "select * from prodotto where categoria = 'Scarpe'";
                        $result = pg_query($dbconn, $q1);
                        $prodotti = pg_fetch_all($result, PGSQL_ASSOC);
                    }
                ...
                if($prodotti) {
                    ...
                    echo '
                        <br>
                        <div class="percorso";>
                            ' . percorsoDiNavigazione($novità, $categoria, $sesso) . ' 
                        </div>';
                    ...
                    foreach ($prodotti as $index => $var) {
                        echo '
                            <div class="card" style="width: 18rem;">
                                ...
                                <img src="immagini/Prodotti/' . $var["immagine1"] . '" class="card-img-top">
                                </div>
                                <div class="card-body">

                                    <p class="card-text font">' . $var["nome"] .'</p>
                                    <p class="card-text font">' . $var["prezzo"] . ' $</p>
                                    <form action="dettagli_prod.php" method="post">
                                        ...
                                        <input type="submit" class="btn btn-dark btn-rounded font" value="Vai al Prodotto">
                                    </form>
    ...
    <?php
        ...
        function percorsoDiNavigazione($novità, $categoria, $sesso) {
            ...
            if ($novità == '' and $categoria == '' and $sesso == '') {
                $s = $s . '<nav style="--bs-breadcrumb-divider: '. "'>'" .';" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                ...
                if($novità == 'N') {
                    ...
                    if($categoria) {
                        $s = $s . '<nav style="--bs-breadcrumb-divider: '. "'>'" .';" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="home.html">Home</a></li>
                                    <li class="breadcrumb-item active" aria-current="page">' . $sesso2 .'</li>
